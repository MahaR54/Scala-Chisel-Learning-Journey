...
class ImmGen extends Module{
    ...
    io.imm:=0.S
     switch(io.ins(6,0)){
        ...
        is(ALU_S){      //S-format
            io.imm:= Cat(Fill(20, io.ins(31)),Cat(io.ins(31,25),io.ins(11,7))).asSInt
        }
        is(ALU_B){      //B-format
            io.imm:= Cat(Cat(io.ins(31),io.ins(7)),Cat(io.ins(30,26),io.ins(11,8))).asSInt
        }
        is(ALU_U){      //U-format
            io.imm:= Cat(Fill(12,0.U),io.ins(19,0)).asSInt
        }
        is(ALU_L){      //L-format
            io.imm:= Cat(Fill(20, io.ins(31)),io.ins(31,20)).asSInt
        }
        is(ALU_J){      //J-format
            io.imm:= Cat(Fill(11,0.U),io.ins(31),io.ins(19,12),io.ins(20),io.ins(30,21),0.U).asSInt  
        }
        is(JAL_R){ 
